...
bool Reader::canBorrow() const
{
    ...
    if (borrowedList.size() == MAX_COPIES)
    {
        return false;
    }
...
void Librarian::deleteUser(std::string name, BST<User *> &list)
{
    ...
    if (!exist)
    {
        std::cerr << "the user with the name does not exist." << std::endl;
        return;
    }
...
void Librarian::deleteBook(int id, BST<Book *> &lib, BST<BookCopy *> &copys)
{
    ...
    if (!exist)
    {
        std::cerr << "the copy with the id does not exist!" << std::endl;
        return;
    }
    ...
    if (deleteCopy.getBook()->sizeCopyList() == 0)
    { // if there is no copy left from that book
